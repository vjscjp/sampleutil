...
func GetHostPort(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Println("___________________  ", resp)
	...
	if resp.App == nil {
		http.Error(w, fmt.Sprintf("Host Port Error: Found Nil Response.App"), http.StatusInternalServerError)
		return
	}
	if resp.Tasks == nil {
		...
		if t.Host == id {
			for _, p := range t.Ports {
				...
				if p == port {
					...
					if err != nil {
						status = http.StatusServiceUnavailable
						controllers.ServeJsonResponseWithCode(w, resp, status)
						return
					}
